...
Every tool name supplied by OMP is a structured tool name, not an Antigravity recipient. Names such as `read`, `glob`, `grep`, `bash`, `edit`, `write`, `task`, `hub`, and `inspect_image` must appear only as names in the terminal `tool_calls` array when that tool is available. Never call Antigravity `send_message` or `manage_inbox` with an OMP tool name as the recipient.
