...
sequenceDiagram
    ...
    B->>B: Validate structured_output
...
Every provider call therefore sends a bounded reconstruction of OMP's current context. This costs more input tokens and process startup time, but it avoids split-brain state.
...
Configure the `task`, `smol`, or other roles explicitly. Selecting `official-agy` only for the parent does not guarantee every OMP child role uses it.
...
1. writes a temporary response schema;
2. passes the prompt using `agy -p <prompt>`;
3. ignores stdin so the child cannot stall waiting for input;
4. parses only stdout NDJSON;
5. retains bounded stderr diagnostics;
6. forwards cancellation with `SIGTERM`, then `SIGKILL` after a grace period;
7. requires exactly one terminal `result` event with `status: SUCCESS`;
